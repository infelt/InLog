...
public class ZipUtils {
     ...
     * 压缩多个文件夹到同一个压缩包
     */
    public static void ZipFolders(String[] srcFiles, String zipFileString) throws Exception {
        if (srcFiles != null && srcFiles.length > 0) {
            //创建ZIP
            ZipOutputStream outZip = new ZipOutputStream(new FileOutputStream(zipFileString));
            for (String srcFileString : srcFiles) {
                //创建文件
                ...
                //压缩
                ZipFiles(file.getParentFile().getParent() + File.separator,
                        file.getParentFile().getName() + File.separator + file.getName(), outZip);
            }
            //完成和关闭
            outZip.finish();
            outZip.close();
